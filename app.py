...
def explore():
    ...
    assets_collection_desc = [
        asset["collection"]["name"] for asset in json_text["assets"]
    ]
    assets_contact_addr = [
        asset["asset_contract"]["address"] for asset in json_text["assets"]
    ]
...
def details():
    ...
    assets_sale_price = float(
        "0." + str(json_text["assets"][0]["last_sale"]["total_price"][:4])
    )
